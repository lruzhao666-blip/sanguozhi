...
// We need to provide a mock environment for main.js to run in
// Actually, it's easier to just read main.js, extract highlightRaw and run it.
const mainCode = fs.readFileSync('js/main.js', 'utf-8');

// The highlightRaw function is global in main.js? No, it's inside window.xx or just declared.
// In main.js, there's `window.highlightRaw = highlightRaw;` at the end or it's a global function.
// Let's create a small runner.
const script = `
  const window = {};
...
▸ 影响：五路计划全面启动\`;
  ...
  console.log(window.highlightRaw(testText));
`;
