...
▸ 影响：五路计划全面启动"""
        ...
        await page.evaluate(f"""(testText) => {{
            window.state.rounds = [{{ round: 1, rawContent: testText }}];
            window.state.currentRoundObj = window.state.rounds[0];
            window.renderAll();
        }}""", test_text)

        # Click the arena tab
        await page.click('button[data-tab="arena"]')
        await page.wait_for_timeout(1000)

        # Get the html of the raw content
        html = await page.evaluate("document.getElementById('arena-raw').innerHTML")
        print(html)
