...
old_func = r'''  // 构建检查 System Prompt（精简版）
...
new_func = r'''  // 构建检查 System Prompt（精简版）
  function _buildCheckSystemPrompt() {
    return [
      '你是《三国志文字版》数据校验助手。检查 GM 输出的数据区是否符合格式规范。',
      '',
...
# 2. replace timeout (specifically the one in _callDataCheckAPI)
# It's currently at around line 572:
#     return fetchWithTimeout(DATA_CHECK_API_URL, {
#       method: 'POST',
# ...
#       })
#     }, 60000).then(function(res) {
old_timeout = r'''        max_tokens: 4000,
        temperature: 0\.3
      \}\)
    \}, 60000\)\.then\(function\(res\) \{'''
new_timeout = r'''        max_tokens: 4000,
        temperature: 0.3
      })
    }, 120000).then(function(res) {'''
text = re.sub(old_timeout, new_timeout, text)
