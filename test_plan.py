...
content = content.replace("padding: 4px 14px;", "padding: 4px 12px;") # Revert msp-city-row issue

match = re.search(r'@media\s*\(max-width:\s*640px\)\s*\{', content)
if match:
    # Ensure no duplicate insertion
    if ".digest-raw .action-player-group" not in content[match.end():match.end()+200]:
