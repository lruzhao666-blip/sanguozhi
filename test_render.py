...
async def main():
    async with async_playwright() as p:
        ...
        # Click the GM tab to make the GM tools visible
        await page.click('button[data-tab="gm"]')
...
▸ 影响：五路计划全面启动"""
        ...
        # Fill the textarea
        await page.fill('#gm-content', test_text)
        ...
        await page.screenshot(path='verification_render.png', full_page=True)

        # Also screenshot just the preview section
        preview_locator = page.locator('#parse-preview')
        if await preview_locator.count() > 0:
            await preview_locator.screenshot(path='verification_preview_only.png')

        print("Render screenshot saved")
