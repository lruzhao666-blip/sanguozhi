/**
 * generals-organize-ui.js — 武将整理 UI 模块 v1
 * 工单 #gen-organizer-v2-step2B
 *
 * 职责：
 *  1. 渲染三张玩家卡内的「武将整理」折叠区
 ...
 *
 * 依赖：
 *  - window.SGGenOrg（generals-organize.js 工单 1）
 *  - window.SGRole.get()（role-login.js）
 *  - window.SGState（main.js）
 *  - #pc-org-body-0/1/2、#pc-org-count-0/1/2（工单 2A HTML）
 *
 * 命名空间：所有 CSS class 使用 .gor-* 前缀，避免冲突
 */
(function () {
  'use strict';
  ...
  // 每个 slot 当前选中的 tab（group_name）
  var _activeTab = { 0: '未分组', 1: '未分组', 2: '未分组' };

  // 长按菜单状态
  var _menuEl = null;     // 当前显示的菜单 DOM
  var _menuSlot = null;
  var _menuGroup = null;
  ...
  function renderSlot(slot) {
    ...
    if (tabNames.indexOf(_activeTab[slot]) === -1) {
      _activeTab[slot] = '未分组';
    }
  ...
  function _renderGroupContent(slot, group, allGroups, editable) {
    ...
    if (editable && !isDefault) {
      ...
      html += '<button class="gor-btn-icon" data-act="group-up" data-slot="' + slot + '" data-group="' + _esc(gName) + '" title="分组左移">◀</button>';
      html += '<button class="gor-btn-icon" data-act="group-down" data-slot="' + slot + '" data-group="' + _esc(gName) + '" title="分组右移">▶</button>';
      html += '<button class="gor-btn-icon gor-btn-danger" data-act="delete" data-slot="' + slot + '" data-group="' + _esc(gName) + '" title="删除分组">🗑</button>';
      html += '</div>';
      html += '</div>';
    }
    ...
    if (editable) {
      ...
      var available = currentGens.filter(function (g) {
        return !inThisGroup[g.name];
      });
  ...
  function _showMenu(slot, groupName, generalName, x, y) {
    ...
    groups.forEach(function (g) {
      if (g.group_name === groupName) currentGroup = g;
    });
    ...
    // 上移/下移
    if (idx > 0) {
      items.push({ label: '↑ 上移', act: 'move-up' });
    }
    if (idx >= 0 && idx < sorted.length - 1) {
      items.push({ label: '↓ 下移', act: 'move-down' });
    }
    ...
    if (groupName !== '未分组') {
      ...
      if (!hasDefault) {
        items.push({ label: '移出到「未分组」', act: 'move-to', target: '未分组' });
      }
    }
    ...
    menu.addEventListener('click', function (ev) {
      ...
      _handleMenuAction(act, btn.getAttribute('data-target'));
    });
  ...
  function _handleMenuAction(act, target) {
    var slot = _menuSlot;
    var group = _menuGroup;
    var gen = _menuGeneral;
    _hideMenu();

    if (!window.SGGenOrg) return;

    if (act === 'move-up') {
      window.SGGenOrg.reorderGeneral(slot, group, gen, 'up').then(function () {
        renderSlot(slot);
      });
    } else if (act === 'move-down') {
      window.SGGenOrg.reorderGeneral(slot, group, gen, 'down').then(function () {
        renderSlot(slot);
      });
    } else if (act === 'move-to' && target) {
      window.SGGenOrg.moveGeneral(slot, gen, group, target).then(function () {
        renderSlot(slot);
      });
    }
  }
  ...
  function _onGroupAction(act, slot, groupName) {
    ...
    if (act === 'rename') {
      ...
      window.SGGenOrg.reorderGroup(slot, groupName, 'up').then(function () {
        renderSlot(slot);
      });

    } else if (act === 'group-down') {
      window.SGGenOrg.reorderGroup(slot, groupName, 'down').then(function () {
        renderSlot(slot);
      });
    }
  }
