...
<div class="raw-player-anchor with-subtitle">
  ...
  // ══════════════════════════════════════════
  /* [legacy v3] 旧版收支渲染(detailMap / totalMap 互斥 bug + 不读 troopOps),由工单 #pcard-changes-render-fix-v1 替代
  ...
  */
  // ══════════════════════════════════════════
  //  收支详情渲染 v4.2 (工单 #pcard-changes-render-fix-v1)
  //  修复点:
  //  A. 同 slot 的 breakdown 与 resources 合并读取,
  //     不再因"互斥分桶"丢失「兵/城/民心」总变化数
  //  B. 兵种△ troopOps 渲染为「兵」行下方的小字明细
  //  C. 括号注解(item.note)在 breakdown 明细中正常显示
  // ══════════════════════════════════════════
  function renderChangesDetail() {
    const latest = state.rounds[state.rounds.length - 1];
    if (!latest) return;
    ...
    // 按 slot 合并:同 slot 的所有 change 对象的字段全部并入 mergedMap[slot]
    // 这样既能读到 breakdown(收支明细行),又能读到 resources(总变化行)
    // 且不会因 parser 把它们拆成两条记录而互相覆盖
    const mergedMap = { '甲': null, '乙': null, '丙': null };
    allChanges.forEach(ch => {
      if (!ch || !ch.slot || !(ch.slot in mergedMap)) return;
      if (!mergedMap[ch.slot]) {
        mergedMap[ch.slot] = {
          slot:       ch.slot,
          breakdown:  {},
          resources:  {},
      ...
      const m = mergedMap[ch.slot];
      // breakdown:逐资源合并(后写覆盖,parser 通常只有一份)
      if (ch.breakdown && typeof ch.breakdown === 'object') {
        Object.keys(ch.breakdown).forEach(k => {
          if (ch.breakdown[k] && Array.isArray(ch.breakdown[k].items) && ch.breakdown[k].items.length) {
            m.breakdown[k] = ch.breakdown[k];
          }
        });
      }
      // resources:逐资源合并(非 null 才覆盖)
      if (ch.resources && typeof ch.resources === 'object') {
        Object.keys(ch.resources).forEach(k => {
          if (ch.resources[k] != null) m.resources[k] = ch.resources[k];
        });
      }
      // troopOps:数组追加
      if (Array.isArray(ch.troopOps) && ch.troopOps.length) {
        m.troopOps = m.troopOps.concat(ch.troopOps);
      }
      // intel:数组追加
      if (Array.isArray(ch.intel) && ch.intel.length) {
        m.intel = m.intel.concat(ch.intel);
      }
    });
    ...
    SLOT_NAMES.forEach((slotName, slotIdx) => {
      ...
      const merged = mergedMap[slotName];
      ...
      // 按"兵种"分组 troopOps:cityName → 数组 [{type, val, isDelta}]
      const troopOpsByCity = {};
      (merged.troopOps || []).forEach(op => {
        if (!op || !op.cityName || !Array.isArray(op.entries)) return;
        if (!troopOpsByCity[op.cityName]) troopOpsByCity[op.cityName] = [];
        op.entries.forEach(e => {
          troopOpsByCity[op.cityName].push({
            type: e.type, val: Number(e.val) || 0, isDelta: !!op.isDelta,
          });
        });
      });
      const hasTroopOps = Object.keys(troopOpsByCity).length > 0;

      const rowsHtml = RES_DEFS.map(def => {
        const bd = merged.breakdown && merged.breakdown[def.key];
        let items = [];
        let sum   = 0;
        let hasDetail = false;
        ...
        if (bd && Array.isArray(bd.items) && bd.items.length) {
          ...
          sum = (typeof bd.total === 'number')
            ...
            : items.reduce((a, b) => a + b.val, 0);
          hasDetail = true;
        } else if (merged.resources && merged.resources[def.key] != null) {
          // 仅总账有值,无明细 —— 修复 bug-A
          sum = Number(merged.resources[def.key]) || 0;
        }
        ...
        if (items.length) {
          ...
          // 无明细:用"无变动"+ 净值占位
          itemsHtml = '<span class="pcc-item"><span class="pcc-label">无变动</span><span class="pcc-val zero">±0</span></span>';
        }

        // 「兵」行特别处理:在主行下方追加 troopOps 子明细(修复 bug-B)
        let troopSubHtml = '';
        if (def.key === '兵' && hasTroopOps) {
          const lines = [];
          Object.keys(troopOpsByCity).forEach(city => {
            const entries = troopOpsByCity[city];
            const segs = entries.map(e => {
              const sign = e.val > 0 ? '+' : '';
              const cls  = e.val > 0 ? 'pos' : (e.val < 0 ? 'neg' : 'zero');
              const valStr = e.isDelta
                ? (e.val === 0 ? '±0' : sign + e.val)
                : ':' + e.val;     // 覆写式:"步:2000"
              return '<span class="pcc-troop-seg"><span class="pcc-troop-type">' +
                     esc(e.type) + '</span>' +
                     '<span class="pcc-troop-val ' + cls + '">' +
                     esc(valStr) + '</span></span>';
            }).join('');
            lines.push('<div class="pcc-troop-line">' +
                       '<span class="pcc-troop-prefix">└</span>' +
                       '<span class="pcc-troop-city">' + esc(city) + '</span>' +
                       '<span class="pcc-troop-segs">' + segs + '</span>' +
                       '</div>');
          });
          troopSubHtml = '<div class="pc-changes-troop-detail">' + lines.join('') + '</div>';
        }
      ...
      // 顶部徽:净金粮变化(优先 resources,回落 breakdown.total)
      let netGold = 0, netFood = 0;
      if (merged.resources['金'] != null) netGold = Number(merged.resources['金']) || 0;
      else if (merged.breakdown['金'] && typeof merged.breakdown['金'].total === 'number') netGold = merged.breakdown['金'].total;
      if (merged.resources['粮'] != null) netFood = Number(merged.resources['粮']) || 0;
      else if (merged.breakdown['粮'] && typeof merged.breakdown['粮'].total === 'number') netFood = merged.breakdown['粮'].total;
      const net = netGold + netFood;
      const netCls = net > 0 ? 'is-positive' : (net < 0 ? 'is-negative' : '');
      const netSign = net > 0 ? '+' : '';
      sumEl.textContent = '净 ' + (net === 0 ? '±0' : netSign + net);
      ...
      // 情报
      if (intelEl) {
        const intelText = (merged.intel || []).filter(Boolean).join(';').trim();
        if (intelText) {
          intelEl.textContent = intelText;
          intelEl.classList.remove('hidden');
        } else {
...
/* ════════════════════════════════════════════════════════════
   v20261111b 工单#pcard-changes-render-fix-v1
   注入兵种子明细的样式 —— 走 JS 注入,不动 css/style.css
   ──────────────────────────────────────────────────────────── */
(function () {
  'use strict';
  if (document.getElementById('sg-pcc-troop-style-v1')) return;
  const style = document.createElement('style');
  style.id = 'sg-pcc-troop-style-v1';
  style.textContent = [
    '.pcard.pcard-v4 .pc-changes-troop-detail{',
    '  padding:1px 13px 5px 38px;',
    '  display:flex;flex-direction:column;gap:2px;',
    '  border-bottom:1px solid rgba(255,255,255,.03);',
    '}',
    '.pcard.pcard-v4 .pc-changes-troop-detail:last-child{',
    '  border-bottom:none;',
    '}',
    '.pcard.pcard-v4 .pcc-troop-line{',
    '  display:flex;align-items:baseline;gap:6px;',
    '  font-size:.66rem;line-height:1.55;',
    '  letter-spacing:.02em;color:var(--text-sub);',
    '  font-family:var(--font-serif);',
    '}',
    '.pcard.pcard-v4 .pcc-troop-prefix{',
    '  color:var(--text-dim);font-size:.62rem;',
    '  flex-shrink:0;opacity:.6;',
    '}',
    '.pcard.pcard-v4 .pcc-troop-city{',
    '  color:var(--gold-dim);font-weight:600;',
    '  flex-shrink:0;letter-spacing:.04em;',
    '}',
    '.pcard.pcard-v4 .pcc-troop-segs{',
    '  display:inline-flex;flex-wrap:wrap;',
    '  gap:2px 10px;align-items:baseline;',
    '}',
    '.pcard.pcard-v4 .pcc-troop-seg{',
    '  display:inline-flex;align-items:baseline;gap:2px;',
    '  white-space:nowrap;',
    '}',
    '.pcard.pcard-v4 .pcc-troop-type{',
    '  color:var(--text-dim);',
    '}',
    '.pcard.pcard-v4 .pcc-troop-val{',
    '  font-family:var(--font-serif);font-weight:500;',
    '  font-feature-settings:"tnum";',
    '}',
    '.pcard.pcard-v4 .pcc-troop-val.pos{color:var(--pos,#8ad99a);}',
    '.pcard.pcard-v4 .pcc-troop-val.neg{color:var(--neg,#e89090);}',
    '.pcard.pcard-v4 .pcc-troop-val.zero{color:var(--text-dim);}',
    '@media(max-width:720px){',
    '  .pcard.pcard-v4 .pc-changes-troop-detail{',
    '    padding:1px 11px 5px 34px;',
    '  }',
    '  .pcard.pcard-v4 .pcc-troop-line{font-size:.62rem;}',
    '}',
  ].join('\n');
