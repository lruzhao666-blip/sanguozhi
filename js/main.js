...
function initRealtime() {
  if (!supabase) {
    console.warn('[Realtime] Supabase 未初始化，降级到轮询模式');
    startPolling();
    return;
  }
  ...
  setTimeout(() => {
    if (!isRealtimeConnected) {
      console.warn('[Realtime] 连接超时，降级到轮询模式');
      startPolling();
    }
  }, 10000);
}
...
function setupRealtimeSubscription() {
  ...
  realtimeChannel = supabase
    .channel('action_submissions')
    .on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'action_submissions_v2'
    }, payload => {
      console.log('[Realtime] 行动提交变化:', payload);
      handleActionSubmissionChange(payload);
    })
    .subscribe((status) => {
      console.log('[Realtime] 行动订阅状态:', status);
      if (status === 'SUBSCRIBED') {
        ...
        startPolling(); // 连接失败，启动轮询
      }
    });

  // 订阅回合表
  realtimeRoundsChannel = supabase
    .channel('sanguo_rounds')
    .on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'sanguo_rounds'
    }, payload => {
      console.log('[Realtime] 新回合发布:', payload);
      handleNewRound(payload);
    })
    .subscribe((status) => {
      console.log('[Realtime] 回合订阅状态:', status);
    });
}
 ...
 * 处理行动提交变化
 */
function handleActionSubmissionChange(payload) {
  // 重新查询当前回合的提交状态
...
async function checkSubmissionStatus() {
  if (!supabase || !window.currentGameState) return;

  const currentRound = window.currentGameState.round;
  if (!currentRound) return;

  try {
    const { data, error } = await supabase
      .from('action_submissions_v2')
      .select('slot, ling_selections, opp_selection')
      .eq('round', currentRound);

    if (error) {
      console.error('[Realtime] 查询提交状态失败:', error);
      return;
    }

    // 统计三家提交情况
    const submissions = { 0: false, 1: false, 2: false };
    if (data) {
      data.forEach(row => {
        const slot = parseInt(row.slot);
        // 判断是否有效提交（ling_selections 或 opp_selection 不为空）
        const hasLing = row.ling_selections && Object.keys(row.ling_selections).length > 0;
        const hasOpp = row.opp_selection && row.opp_selection !== 'EMPTY';
        submissions[slot] = hasLing || hasOpp;
      });
    }

    // 更新 UI
    updateSubmissionUI(submissions);

    // 检查是否三家齐全
    const allSubmitted = submissions[0] && submissions[1] && submissions[2];
    if (allSubmitted && !lastSubmissionCheck.allSubmitted) {
      showAllSubmittedBanner();
    }

    lastSubmissionCheck = { ...submissions, allSubmitted };

  } catch (err) {
    console.error('[Realtime] 检查提交状态异常:', err);
  }
}
...
function updateSubmissionUI(submissions) {
  const markersEl = document.getElementById('submit-markers');
  const countEl = document.getElementById('submit-count');
  const statusEl = document.getElementById('submit-status');
  const sepEl = document.getElementById('submit-status-sep');

  if (!markersEl || !countEl || !statusEl) return;

  // 如果不在行动 Tab，隐藏状态
  const actionTab = document.getElementById('tab-action');
  if (!actionTab || !actionTab.classList.contains('active')) {
  ...
  slots.forEach((name, idx) => {
    const done = submissions[idx];
    if (done) count++;
    const cls = done ? 'done' : 'pending';
    const mark = done ? '✓' : '—';
    html += `<span class="submit-marker ${cls}">${name}${mark}</span>`;
  });
 ...
 * 显示"三家已齐"横幅
 */
function showAllSubmittedBanner() {
  showRealtimeBanner('✓', '三家行动已齐 · 可通知 GM 结算', 'success', 5000);
}
...
function handleNewRound(payload) {
  ...
  const roundNum = newRound.round_number || newRound.round || '新';

  // 显示横幅 + 倒计时刷新
  showNewRoundBanner(roundNum);
}

/**
 * 显示新回合横幅（带倒计时）
 */
function showNewRoundBanner(roundNum) {
  const banner = document.getElementById('realtime-banner');
  if (!banner) return;

  let countdown = 3;
  const iconEl = banner.querySelector('.rb-icon');
  const textEl = banner.querySelector('.rb-text');

  banner.className = 'realtime-banner new-round show';
  iconEl.textContent = '⚔️';

  const updateText = () => {
    textEl.textContent = `第 ${roundNum} 回合已发布 · ${countdown} 秒后自动刷新`;
  };
 ...
 * 显示实时通知横幅（通用）
 */
function showRealtimeBanner(icon, text, type = 'success', duration = 5000) {
  const banner = document.getElementById('realtime-banner');
  if (!banner) return;
  ...
  banner.className = 'realtime-banner show';
...
function updateSyncStatus(status) {
  ...
  const textEl = statusEl ? statusEl.querySelector('.sync-text') : null;
  ...
  if (status === 'connected') {
    ...
    textEl.textContent = '连接断开 · 点击刷新';
    statusEl.onclick = () => location.reload();
  }
}

/**
 * 启动轮询模式（Realtime 失败时的兜底）
 */
function startPolling() {
  if (pollInterval) return; // 避免重复启动

  console.log('[Polling] 启动轮询模式，每 10 秒检查一次');
...
window.addEventListener('beforeunload', () => {
  ...
  if (realtimeRoundsChannel) {
    supabase.removeChannel(realtimeRoundsChannel);
  }
  stopPolling();
});
