...
(function () {
  ...
  function bindGMPanel() {
    ...
    document.getElementById('pg-btn-generate').addEventListener('click', onGeneratePrompt);
    document.getElementById('pg-btn-copy').addEventListener('click', onCopyPrompt);
  }
  ...
  function onGeneratePrompt() {
    const btnGen = document.getElementById('pg-btn-generate');
    const btnCopy = document.getElementById('pg-btn-copy');
    const actionsTa = document.getElementById('pg-actions');
    const outputTa = document.getElementById('pg-output');

    if (state.rounds.length === 0) {
      showToast('⚠️ 请先发布第 1 回合后再使用此功能');
      btnGen.disabled = true;
      return;
    }

    const latestRound = state.rounds[state.rounds.length - 1];
    const rawContent = latestRound.rawContent;

    if (!rawContent) {
      showToast('⚠️ 未找到有效数据区,请检查上一回合发布内容');
      return;
    }

    const sep = '='.repeat(36);
    const codeM = rawContent.match(/```[\w]*\n?([\s\S]*?)```/);
    const codeBlock = codeM ? codeM[1] : rawContent;
    ...
    let actionsText = actionsTa.value.trim();
    if (!actionsText) {
      if (!confirm('⚠️ 玩家行动为空,将生成空行动 Prompt,确认?')) return;
    }

    // N 的取值: 从上一回合数据区 [回合] 行中提取数字
    let nextRoundNum = '?';
    const roundMatch = dataZone.match(/\[回合\]\s*第\s*(\d+)\s*回合/);
    if (roundMatch && roundMatch[1]) {
      nextRoundNum = parseInt(roundMatch[1], 10) + 1;
    } else {
      nextRoundNum = latestRound.round + 1; // fallback using round from state
    }

    // Dice helper using crypto.getRandomValues
    function rollD6() {
      const arr = new Uint32Array(1);
      crypto.getRandomValues(arr);
      return Math.floor((arr[0] / (0xFFFFFFFF + 1)) * 6) + 1;
    }

    const d3_5 = Array.from({length: 5}, () => [rollD6(), rollD6(), rollD6()]);
    const d2_3 = Array.from({length: 3}, () => [rollD6(), rollD6()]);
    const d1_4 = Array.from({length: 4}, () => rollD6());

    const template = `----------模板开始----------
=== 第 ${nextRoundNum} 回合状态快照(权威数据,以此为准)===
...
请基于上方状态快照与玩家行动,生成下一回合完整输出。
本回合所有掷骰必须从上面的随机数池中按顺序取用,
严禁使用字数定骰公式,严禁自己编造骰点。骰式行必须
公开写出每颗骰子的具体点数(即池中的具体数字)。
----------模板结束----------`;

    outputTa.value = template;
  ...
  async function onCopyPrompt() {
    ...
    try {
      ...
      outputTa.select();
      showToast('⚠️ 自动复制失败,请手动按 Ctrl+C 复制');
    }
  }
