...
<div class="raw-player-anchor with-subtitle">
  ...
  // ══════════════════════════════════════════
  //  治政工程数据反推引擎 v1
  //  数据源:state.rounds 全历史 → changes[].breakdown[].items[].note
  //  注解格式:`{工程名}兴-{城名}` 投入 / `{工程名}成-{城名}` 到期
  // ══════════════════════════════════════════
  function _buildGongchengData() {
    const result = { 0: { ongoing: [], completed: [] },
                     1: { ongoing: [], completed: [] },
                     2: { ongoing: [], completed: [] } };
    if (!state.rounds.length) return result;

    const SLOT_MAP = { '甲': 0, '乙': 1, '丙': 2 };
    const currentRound = state.rounds[state.rounds.length - 1].round || 0;

    // 临时存储:key = `${slot}|${city}|${name}|${startRound}`
    // 用于配对投入/到期
    const projectMap = {};

    state.rounds.forEach(rd => {
      const roundNum = rd.round || 0;
      const changes = (rd.parsed && rd.parsed.changes) || [];
      changes.forEach(ch => {
        if (!ch || !ch.slot || !(ch.slot in SLOT_MAP)) return;
        const slot = SLOT_MAP[ch.slot];
        if (!ch.breakdown) return;

        // 扫金粮两栏的 items
        ['金', '粮'].forEach(resType => {
          const bd = ch.breakdown[resType];
          if (!bd || !Array.isArray(bd.items)) return;
          bd.items.forEach(item => {
            if (!item.note) return;
            // 匹配 投入:`XX兴-城名` / 到期:`XX成-城名`
            // 兼容多个工程合并写法(用 + 连接),按 + 拆分
            const segs = item.note.split('+');
            segs.forEach(seg => {
              const startM = seg.match(/^(.+?)兴-(.+?)$/);
              const endM   = seg.match(/^(.+?)成-(.+?)$/);

              if (startM) {
                const projName = startM[1].trim();
                const cityName = startM[2].trim();
                if (!projName || !cityName) return;
                // 用 城+工程名 作为 key 后缀(同名工程不可在同城重叠投入)
                const baseKey = `${slot}|${cityName}|${projName}`;
                // 若已存在未结束的同 key 投入,跳过(避免重复)
                const existing = Object.keys(projectMap).find(k =>
                  k.startsWith(baseKey + '|') && !projectMap[k].completed);
                if (existing) return;
                const key = `${baseKey}|${roundNum}`;
                if (!projectMap[key]) {
                  projectMap[key] = {
                    slot, city: cityName, name: projName,
                    startRound: roundNum,
                    investGold: 0, investFood: 0,
                    completed: false, completedRound: null,
                  };
                }
                // 累加该资源类型的投入额(取绝对值)
                if (resType === '金') projectMap[key].investGold += Math.abs(item.val) || 0;
                if (resType === '粮') projectMap[key].investFood += Math.abs(item.val) || 0;
              }
              if (endM) {
                const projName = endM[1].trim();
                const cityName = endM[2].trim();
                if (!projName || !cityName) return;
                const baseKey = `${slot}|${cityName}|${projName}`;
                // 找到最早一笔未完成的同 key 投入
                const matchKey = Object.keys(projectMap)
                  .filter(k => k.startsWith(baseKey + '|') && !projectMap[k].completed)
                  .sort()[0];
                if (matchKey) {
                  projectMap[matchKey].completed = true;
                  projectMap[matchKey].completedRound = roundNum;
                }
              }
            });
          });
    ...
    // 整理为 ongoing / completed 两个数组
    Object.values(projectMap).forEach(p => {
      // 档位判定
      const goldTier = p.investGold <= 100 ? 0 : p.investGold <= 300 ? 1 : 2;
      const foodTier = p.investFood <= 200 ? 0 : p.investFood <= 600 ? 1 : 2;
      const tierIdx = Math.max(goldTier, foodTier);
      const tierName = ['small','mid','big'][tierIdx];
      const tierCycle = [3, 5, 8][tierIdx];
      const tierLabel = ['小','中','大'][tierIdx];
      p.tier = tierName;
      p.tierCycle = tierCycle;
      p.tierLabel = tierLabel;

      if (p.completed) {
        result[p.slot].completed.push(p);
      } else {
        // 中断检测:超过周期 + 2 回合容差仍未到期 → 从在建列表移除
        const elapsed = currentRound - p.startRound;
        if (elapsed > tierCycle + 2) return;
        const remaining = Math.max(0, tierCycle - elapsed);
        p.elapsed = elapsed;
        p.remaining = remaining;
        result[p.slot].ongoing.push(p);
      }
    });
  ...
  //  渲染单个玩家卡的治政工程折叠区
  // ══════════════════════════════════════════
  function renderGongcheng() {
    const data = _buildGongchengData();
    for (let slot = 0; slot < 3; slot++) {
      const wrap = document.getElementById('pc-gongcheng-' + slot);
      if (!wrap) continue;
      const cntEl = wrap.querySelector('.pcs-count');
      const listEl = wrap.querySelector('.pc-gongcheng-list');
      if (!cntEl || !listEl) continue;

      const ongoing = data[slot].ongoing;
      const completed = data[slot].completed;

      // 计数
      cntEl.textContent = ongoing.length + ' 在建 · ' + completed.length + ' 已成';

      // 整段空态:在建+已成均为 0 时,折叠区整体隐藏
      ...
      let html = '';
      if (ongoing.length) {
        ongoing.sort((a, b) => a.remaining - b.remaining);
        ongoing.forEach(p => {
          const urgent = p.remaining <= 1 ? ' urgent' : '';
          // 进度条单元格
          let cellsHtml = '';
          for (let i = 0; i < p.tierCycle; i++) {
            cellsHtml += '<span class="gc-progress-cell' + (i < p.elapsed ? ' done' : '') + '"></span>';
          }
          // 投入资源 chips
          let costHtml = '';
          if (p.investGold > 0) costHtml += '<span class="gc-cost-gold">-' + p.investGold + '金</span>';
          if (p.investGold > 0 && p.investFood > 0) costHtml += '<span class="gc-cost-sep">·</span>';
          if (p.investFood > 0) costHtml += '<span class="gc-cost-food">-' + p.investFood + '粮</span>';
          html += '<div class="gc-row" data-tier="' + p.tier + '">' +
            '<div class="gc-main"><span class="gc-city-project">' +
              '<span class="gc-city">' + esc(p.city) + '</span>' +
              '<span class="gc-sep">·</span>' +
              '<span class="gc-project">' + esc(p.name) + '</span>' +
            '</span></div>' +
            '<span class="gc-tier-badge">' + p.tierLabel + '</span>' +
            '<span class="gc-cost">' + costHtml + '</span>' +
            '<div class="gc-progress">' +
              '<span class="gc-progress-text' + urgent + '">剩 ' + p.remaining + ' / ' + p.tierCycle + '</span>' +
              '<span class="gc-progress-bar">' + cellsHtml + '</span>' +
            '</div>' +
          '</div>';
        });
      } else {
        html += '<div class="gc-empty">本家暂无在建工程</div>';
      }

      // 已建成设施嵌套折叠
      if (completed.length) {
        completed.sort((a, b) => (b.completedRound || 0) - (a.completedRound || 0));
        let itemsHtml = '';
        completed.forEach(p => {
          itemsHtml += '<span class="gc-completed-item">' +
            '<span class="gc-completed-city">' + esc(p.city) + '</span>' +
            '<span class="gc-completed-sep">·</span>' +
            esc(p.name) +
          '</span>';
        });
        html += '<details class="gc-completed-nest">' +
          '<summary>已建成设施<span class="gc-completed-badge-mini">' + completed.length + ' 项</span></summary>' +
          '<div class="gc-completed-list">' + itemsHtml + '</div>' +
        '</details>';
      }
