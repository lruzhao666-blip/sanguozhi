...
/* ════════════════════════════════════════════════
   战争迷雾模块 v1  #fog-of-war-v1
   ────────────────────────────────────────────────
   职责:
   - 监听 sg-role-changed / sg-rounds-updated 事件
   - 根据当前身份(SGRole.get())+ GM 关闭开关 + 是否 GM 模式
     决定是否给 body 加 .fog-active class
   - 给 B/C 玩家卡 + 地图城节点 + 城悬浮卡的 DOM 追加
     data-fog="other" 标记,纯 CSS 接管模糊渲染
   - GM 关闭开关状态存 localStorage[sg-fog-disabled]
   - 完全不改任何数据流,只在 DOM 渲染完成后追加属性
   ════════════════════════════════════════════════ */
(function () {
  'use strict';

  const FOG_KEY = 'sg-fog-disabled';
  const SLOT_TO_IDX = { '甲': 0, '乙': 1, '丙': 2 };

  function isGMMode() {
    return document.body.classList.contains('is-gm-mode');
  }

  function isFogDisabled() {
    if (!isGMMode()) return false;  // 只有 GM 才能"关闭迷雾"
    try {
      return localStorage.getItem(FOG_KEY) === '1';
    } catch (e) { return false; }
  }

  function setFogDisabled(disabled) {
    try {
      if (disabled) localStorage.setItem(FOG_KEY, '1');
      else          localStorage.removeItem(FOG_KEY);
    } catch (e) {}
  }
  ...
  function shouldActivate() {
    const role = (window.SGRole && typeof window.SGRole.get === 'function')
      ? window.SGRole.get() : null;
    // 未登录:不激活迷雾,看全貌
    if (!role || !(role in SLOT_TO_IDX)) return false;
    // GM 模式 + 已勾选"关闭迷雾":不激活
    if (isGMMode() && isFogDisabled()) return false;
    // 其他情况:激活
    return true;
  }

  /* 取当前身份对应的 slot 索引(0/1/2),无身份返回 -1 */
  function getMySlot() {
    const role = (window.SGRole && typeof window.SGRole.get === 'function')
      ? window.SGRole.get() : null;
    if (!role || !(role in SLOT_TO_IDX)) return -1;
    return SLOT_TO_IDX[role];
  }

  /* 给玩家卡 DOM 追加 data-fog 标记 */
  function tagPlayerCards(mySlot) {
    for (let i = 0; i < 3; i++) {
      // 玩家卡根容器:站点结构里每张卡有 id 模式 pname-N 等;
      // 我们通过 pname-N 节点向上找最近的玩家卡根容器
      const nameEl = document.getElementById('pname-' + i);
      if (!nameEl) continue;
      // 向上找带 .player-card 或类似根 class 的容器;
      // 若找不到则给 nameEl 的祖父节点打标(兜底)
      const card = nameEl.closest('[data-player-card], .player-card, .pcard, .arena-player-card')
                 || nameEl.parentElement && nameEl.parentElement.parentElement;
      if (!card) continue;
      if (i === mySlot) {
        card.removeAttribute('data-fog');
      } else {
        card.setAttribute('data-fog', 'other');
      }
    }
  }

  /* 给地图上的城节点追加 data-fog 标记
     地图节点通常带 data-city / data-player-idx 之类属性,
     或在 SVG 内有 class 标识。这里用宽松匹配多种可能。 */
  function tagMapCities(mySlot) {
    // 找所有带 data-player-idx 的城节点(玩家城)
    const nodes = document.querySelectorAll('[data-player-idx]');
    nodes.forEach(node => {
      const idx = parseInt(node.getAttribute('data-player-idx'), 10);
      if (!Number.isInteger(idx) || idx < 0) return;
      if (idx === mySlot) {
        node.removeAttribute('data-fog');
      } else if (idx === 0 || idx === 1 || idx === 2) {
        node.setAttribute('data-fog', 'other');
      }
    });
    // NPC 城不参与迷雾(playerIdx === -1 或无该属性的节点跳过)
  }

  /* 给城悬浮卡追加 data-fog 标记
     悬浮卡是按需弹出的,这里用 MutationObserver 监听 DOM 插入。
     找到带 data-tooltip-player-idx 或类似标识的悬浮卡时打标。 */
  function tagTooltipIfNeeded(tooltipEl, mySlot) {
    if (!tooltipEl || !tooltipEl.getAttribute) return;
    // 尝试多种属性名兼容(根据 map.js 实际渲染调整)
    const pidxAttr = tooltipEl.getAttribute('data-player-idx')
                  || tooltipEl.getAttribute('data-tooltip-player-idx');
    if (pidxAttr === null) return;
    const idx = parseInt(pidxAttr, 10);
    if (!Number.isInteger(idx) || idx < 0) return;
    if (idx === mySlot) {
      tooltipEl.removeAttribute('data-fog');
    } else if (idx === 0 || idx === 1 || idx === 2) {
      tooltipEl.setAttribute('data-fog', 'other');
    }
  }
  ...
  function apply() {
    ...
    document.body.classList.toggle('fog-active', active);

    // 顶部状态条
    const banner = document.getElementById('fog-banner');
    if (banner) {
      if (active) {
    ...
    if (!active) {
      // 关闭迷雾:清除所有 data-fog 标记
      document.querySelectorAll('[data-fog="other"]').forEach(el => {
    ...
    if (mySlot < 0) return;
    tagPlayerCards(mySlot);
    tagMapCities(mySlot);
    // 已存在的悬浮卡也一并打标(若有)
    document.querySelectorAll('[data-player-idx],[data-tooltip-player-idx]').forEach(el => {
      tagTooltipIfNeeded(el, mySlot);
    });
  }

  /* 绑定 GM 开关 */
  function bindGmToggle() {
    const cb = document.getElementById('fog-disable-checkbox');
    if (!cb) return;
    // 初始化勾选态
    cb.checked = isFogDisabled();
    cb.addEventListener('change', function () {
      setFogDisabled(cb.checked);
      apply();
    });
  }

  /* MutationObserver:监听新增到 body 的悬浮卡节点,自动打标 */
    ...
    const obs = new MutationObserver(function (mutations) {
      ...
      mutations.forEach(m => {
        m.addedNodes.forEach(node => {
          ...
          // 节点自身
          tagTooltipIfNeeded(node, mySlot);
          // 子孙节点
          node.querySelectorAll && node.querySelectorAll('[data-player-idx],[data-tooltip-player-idx]')
            .forEach(el => tagTooltipIfNeeded(el, mySlot));
        });
      });
    });
  ...
  function init() {
    ...
    apply();
    // 监听全局事件
    window.addEventListener('sg-role-changed', apply);
    window.addEventListener('sg-rounds-updated', function () {
      // 回合更新会重渲玩家卡和地图,延迟一帧再打标
      setTimeout(apply, 50);
    });
  }
  ...
  window.SGFog = {
    ...
    disable: function () { setFogDisabled(true); apply(); },
    enable:  function () { setFogDisabled(false); apply(); },
  };
})();
